...
int _do_output(char *str, const char *format, va_list p, fmt_t *fmt)
{
    ...
    {
        printf("\nbegin: %p, char: %c\nend: %p, char: %c\nspec: %c", fmt->begin,
               *(fmt->begin), fmt->end, *(fmt->end),
               fmt->specifier); // TODO Remove
...
char *_int_to_str(va_list p, fmt_t *fmt)
{
    ...
    *res = '\0';
    ...
    {
        for (int i = 0; i < fmt->precision - s21_strlen(str); i++)
            s21_strcat(res, "0");
    }
    s21_strcat(res, str);
    free(str);
