...
START_TEST(upper_hex_base_version) {
    uint32_t a1, a2;
    const char str[] = "F";
    const char fstr[] = "%X";
...
START_TEST(upper_hex_spaces_tabs_sns) {
    uint32_t a1, a2;
    const char str[] = "          \n             \n     F";
    const char fstr[] = "%X";
    uint32_t res1 = s21_sscanf(str, fstr, &a1);
...
Suite *make_sscanf_suite(void) {
    ...
    tcase_add_test(tc, chars_flags3);  // Demonstrates how (*) works. * - ignore the char
...
//    tcase_add_test(tc, spec_i_hex2);
    ...
    // [%s]
    // Width: (N) - parses N chars, (*) - skip string
    // OUT: res = 4 (correctly parses everything into different strings)
...
//    tcase_add_test(tc, n5);
    ...
    tcase_add_test(tc, upper_hex_base_version);
    tcase_add_test(tc, upper_hex_overflow);
    tcase_add_test(tc, upper_hex_0x);
    tcase_add_test(tc, upper_hex_0X);
    tcase_add_test(tc, upper_hex_empty);
//    tcase_add_test(tc, upper_hex_fail);
    tcase_add_test(tc, upper_hex_spaces_tabs_sns);
    tcase_add_test(tc, upper_hex_short);
    tcase_add_test(tc, upper_hex_long);
//    tcase_add_test(tc, upper_hex_longlong);
    tcase_add_test(tc, upper_hex_2x);
    tcase_add_test(tc, upper_hex_star);
//    tcase_add_test(tc, upper_hex_nohex);
    tcase_add_test(tc, upper_hex_lower);
    tcase_add_test(tc, upper_hex_sign);
